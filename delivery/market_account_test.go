/*
FILE: delivery/market_account_test.go
...
func TestGetContract_ParsesSpec(t *testing.T) {
	...
	var srv *httptest.Server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotPath = r.URL.Path
		// Delivery contracts are DATED and carry expire_time/cycle (no funding).
		_, _ = io.WriteString(w, `{"name":"BTC_USDT_20240329","type":"direct","quanto_multiplier":"0.0001",
			...
			"expire_time":1711699200,"cycle":"QUARTERLY","in_delisting":false}`)
	}))
	defer srv.Close()

	var fut *Client = newDeliveryTestClient(t, srv.URL)
	var info types.SymbolInfo
	var err error
	info, err = fut.MarketData().GetContract(context.Background(), "BTC_USDT_20240329")
	if err != nil {
		t.Fatalf("GetContract: %v", err)
	}
	if gotPath != "/delivery/usdt/contracts/BTC_USDT_20240329" {
	...
	if info.OrdersLimit != 100 {
		t.Fatalf("orders: %d", info.OrdersLimit)
	}
	// Delivery-specific: expire_time (seconds) normalized to ms, plus cycle.
	if info.ExpireTimeMs != 1711699200000 || info.Cycle != "QUARTERLY" {
		t.Fatalf("expiry/cycle: %d / %q", info.ExpireTimeMs, info.Cycle)
	}
}
...
// TestGetSettlements_ParsesRecords covers the delivery-specific settlements
// endpoint (no perpetual-futures analogue).
func TestGetSettlements_ParsesRecords(t *testing.T) {
	var gotPath, gotQuery string
	var srv *httptest.Server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotPath, gotQuery = r.URL.Path, r.URL.RawQuery
		_, _ = io.WriteString(w, `[{"time":1711699200,"contract":"BTC_USDT_20240329","size":-5,
			"leverage":"10","margin":"150","profit":"42.5","pnl":"40","fee":"0.3","settle_price":"30500"}]`)
	}))
	defer srv.Close()

	var fut *Client = newDeliveryTestClient(t, srv.URL)
	var recs []types.Settlement
	var err error
	recs, err = fut.Account().GetSettlements(context.Background(), "BTC_USDT_20240329", 50)
	if err != nil {
		t.Fatalf("GetSettlements: %v", err)
	}
	if gotPath != "/delivery/usdt/settlements" {
		t.Fatalf("path: %q", gotPath)
	}
	if !strings.Contains(gotQuery, "contract=BTC_USDT_20240329") || !strings.Contains(gotQuery, "limit=50") {
		t.Fatalf("query: %q", gotQuery)
	}
	if len(recs) != 1 {
		t.Fatalf("expected 1 settlement, got %d", len(recs))
	}
	if recs[0].Contract != "BTC_USDT_20240329" || recs[0].Side != types.SideTypeSell || !recs[0].Size.Equal(mustDec("5")) {
		t.Fatalf("record: %+v", recs[0])
	}
	if recs[0].TimeMs != 1711699200000 || !recs[0].SettlePrice.Equal(mustDec("30500")) || !recs[0].Profit.Equal(mustDec("42.5")) {
		t.Fatalf("time/price/profit: %d / %s / %s", recs[0].TimeMs, recs[0].SettlePrice, recs[0].Profit)
	}
}
...
func TestSetLeverage_QueryEncoding(t *testing.T) {
	...
	if gotMethod != http.MethodPost || gotPath != "/delivery/usdt/positions/BTC_USDT/leverage" {
...
func TestSetPositionMode_DualModeQuery(t *testing.T) {
	...
	if gotPath != "/delivery/usdt/dual_mode" || gotQuery != "dual_mode=false" {
...
func TestClosePosition_IssuesMarketClose(t *testing.T) {
	...
	if gotPath != "/delivery/usdt/orders" {
		t.Fatalf("path: %q", gotPath)
	}
