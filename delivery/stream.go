/*
FILE: delivery/stream.go

DESCRIPTION:
WebSocket subscription sub-client for the Gate Delivery section. One shared
...
WatchLastPrice/WatchIndexPrice (tickers), WatchTrades, WatchOrderBook (L2 via
the shared orderbook engine). Private: WatchOrders, WatchPositions (require
credentials; payload needs the account user id, fetched once lazily from
GET /delivery/{settle}/accounts).

WS HOST & CHANNELS (calibration): the delivery socket is a delivery-specific
host (Config.WS.DeliveryURL, default wss://fx-ws.gateio.ws/v4/ws/delivery/usdt);
Gate routes delivery by CONNECTION, reusing the "futures.*" channel namespace on
that socket. Confirm host + channel prefix live.
...
// Delivery WS channels. Gate reuses the "futures." channel namespace on the
// delivery socket (the host, not the prefix, routes to delivery). CALIBRATION:
// confirm the prefix live — switch to "delivery." here if Gate uses that.
const (
...
// ensureUserID fetches and caches the account user id (GET /delivery/{settle}/accounts).
