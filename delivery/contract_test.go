/*
FILE: delivery/contract_test.go
...
func newDeliveryTestClient(t *testing.T, baseURL string) *Client {
	t.Helper()
...
func TestCreateOrder_LimitBuy_Encoding(t *testing.T) {
	...
	if gotMethod != http.MethodPost || gotPath != "/delivery/usdt/orders" {
...
func TestCancelOrder_ByClientID_PathAndMethod(t *testing.T) {
	...
	if gotMethod != http.MethodDelete || gotPath != "/delivery/usdt/orders/t-myid" {
...
func TestCancelAllOrders_NativeDelete(t *testing.T) {
	...
	if gotMethod != http.MethodDelete || gotPath != "/delivery/usdt/orders" {
		t.Fatalf("unexpected request: %s %s", gotMethod, gotPath)
	}
