/*
FILE: options/trading_validation_test.go
...
client-id "t-" format, and amend identifier/field/side rules.
*/
...
func buildClient() *Client {
	...
	parent, _ = gate.NewClient(gate.Config{})
...
func TestBuildCreate_MissingSide(t *testing.T) {
	...
	var _, err = tc.buildCreateOrderBody(types.CreateOrderRequest{
		Contract: testContract, Size: mustDec("1"), Price: mustDec("10"),
	})
...
func TestBuildCreate_NonIntegerSize(t *testing.T) {
	var tc *TradingClient = buildClient().Trading()
	var _, err = tc.buildCreateOrderBody(types.CreateOrderRequest{
		Contract: testContract, Side: types.SideTypeBuy, Size: mustDec("1.5"), Price: mustDec("10"),
	})
	if err == nil || !gate.IsInvalidRequest(err) {
...
func TestBuildCreate_LimitRequiresPrice(t *testing.T) {
	var tc *TradingClient = buildClient().Trading()
	var _, err = tc.buildCreateOrderBody(types.CreateOrderRequest{
		Contract: testContract, Side: types.SideTypeBuy, Size: mustDec("1"), OrderType: types.OrderTypeLimit,
	})
	if err == nil || !gate.IsInvalidRequest(err) {
...
func TestBuildCreate_MarketBadTIF(t *testing.T) {
	var tc *TradingClient = buildClient().Trading()
	var _, err = tc.buildCreateOrderBody(types.CreateOrderRequest{
		Contract: testContract, Side: types.SideTypeBuy, Size: mustDec("1"),
...
func TestBuildCreate_MarketSellSignedSize(t *testing.T) {
	var tc *TradingClient = buildClient().Trading()
	var body, err = tc.buildCreateOrderBody(types.CreateOrderRequest{
		Contract: testContract, Side: types.SideTypeSell, Size: mustDec("4"), OrderType: types.OrderTypeMarket,
	})
	if err != nil {
		t.Fatalf("market sell should build, got %v", err)
	}
	if body["size"].(int64) != -4 {
		t.Fatalf("market sell size: got %v want -4", body["size"])
	}
	if body["price"] != "0" || body["tif"] != "ioc" {
		t.Fatalf("market shaping: price=%v tif=%v", body["price"], body["tif"])
	}
}
...
func TestBuildCreate_CloseZeroSize(t *testing.T) {
	...
	var body, err = tc.buildCreateOrderBody(types.CreateOrderRequest{
		Contract: testContract, Close: true, Price: mustDec("10"),
	})
	if err != nil {
...
func TestBuildAmend_RequiresOneIdentifier(t *testing.T) {
	var _, err = buildAmendBody(types.ModifyOrderRequest{NewPrice: mustDec("10")})
	if err == nil || !gate.IsInvalidRequest(err) {
		t.Fatalf("expected InvalidRequest with no identifier, got %v", err)
	}
	_, err = buildAmendBody(types.ModifyOrderRequest{OrderID: "1", ClientOrderID: "t-a", NewPrice: mustDec("10")})
	if err == nil || !gate.IsInvalidRequest(err) {
