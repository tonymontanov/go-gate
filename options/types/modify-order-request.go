/*
FILE: options/types/modify-order-request.go

DESCRIPTION:
ModifyOrderRequest — input for TradingClient.ModifyOrder
(Gate PUT /options/orders/{order_id}).
...
convention (the new size must keep the original direction) — exactly like the
futures section.
*/
...
// ModifyOrderRequest — parameters for amending an options order. Exactly one of
