...
DESCRIPTION:
Market-data sub-client for the Gate Futures section. Implements (all public, no
signing):
  - GetContracts / GetContract : GET /futures/{settle}/contracts[/{contract}] → SymbolInfo
  - GetOrderBook               : GET /futures/{settle}/order_book (with_id=true)
  - GetCandlesticks            : GET /futures/{settle}/candlesticks
  - GetTickers                 : GET /futures/{settle}/tickers
...
func symbolInfoFromPayload(p *contractPayload, rateLimits map[string]string) types.SymbolInfo {
	return types.SymbolInfo{
		...
		FundingIntervalSec: p.FundingInterval,
